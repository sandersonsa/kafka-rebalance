...
public class ProducerService {
    ...
    @Value("${app.topico.entrada}")
    String topicoEntrada;
    ...
    public CompletableFuture<String> sendMessages(String uuid) throws InterruptedException {                
        kafkaTemplate.send(topicoEntrada, uuid);
        return CompletableFuture.completedFuture("Complete");
    }

    @Async
    public CompletableFuture<String> sendMessagesJson023(String uuid) throws InterruptedException {                
        JSONObject json = buildJson023(uuid, getRenavam());

        var record = new ProducerRecord<String, String>(topicoEntrada, json.toString());
    ...
    @Async
    public CompletableFuture<String> sendMessagesJson017(String uuid) throws InterruptedException {                
        JSONObject json = buildJson017(uuid, getRenavam());

        var record = new ProducerRecord<String, String>(topicoEntrada, json.toString());
        record.headers().add("transaction-id", "17".getBytes());
    ...
    private JSONObject buildJson023(String id, String renavam) {
    ...
    private JSONObject buildJson017(String id, String renavam) {
        JSONObject json = new JSONObject();
        ...
        parteFixa.put("sequencial", "003002");
        parteFixa.put("modalidade", 4);
        parteFixa.put("codigo_transacao", "017");
        parteFixa.put("uf_origem_transacao", "SF");
        parteFixa.put("uf_origem_transmissao", "SF");
        parteFixa.put("uf_destino_transmissao", "RJ");
        parteFixa.put("tipo_condicionalidade", 1);
        parteFixa.put("tamanho_transacao", "0016");
        parteFixa.put("codigo_retorno_transacao", "00");
        parteFixa.put("dia_juliano", "346");
        json.put("parte_fixa", parteFixa);
        ...
        JSONObject parteVariavel = new JSONObject();
        parteVariavel.put("renavam", renavam);
        parteVariavel.put("opcao", 2);
        parteVariavel.put("exercicio", "2024");
